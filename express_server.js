...
const bodyParser = require("body-parser"); //convert request body from Buffer into readable string and add data to the req(request) object under the key body. 
...
//Passing user Object to the _header, find endpoints for templates, define template Vars there to render 
...
app.get("/urls", (req, res) => { //pass the URL data to our template urls_index.ejs in views folder
  let userID = req.session.user_id; //check cookies for userID, if have
  if (userID){
...
app.post('/login', (req, res) => { // post req with body, extract info from form 
  ...
  //password is salted  
    ...
    req.session.user_id //setting cookie with user ID
    //console.log(users); //round brackets
    res.redirect("/urls");
  } else {
    // send 403
...
app.post('/logout', (req, res) => { // post req with body
  ...
  req.session = null;
  res.redirect('/login');
  //res.redirect('/urls'); //Previous redirect back to list of URLs. Can't be longURL cause redirect to whatever put in
});
...
app.post('/urls/:shortURL/delete', (req, res) => { //SAVE longURL from short
  ...
  const shortUrl = req.params.shortURL; //we get longURL from dbase, access property of object (dbase), shortURL is key   
...
//Curl validate if URL is saved in dbase; Browser validate if redirect is occurring